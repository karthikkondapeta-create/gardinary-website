...
export default function Marquee() {
  const loopWords = Array(10).fill(words).flat()
    ...
    <div className="bg-forest-700 border-y border-forest-600 overflow-hidden py-4">
      <div className="flex whitespace-nowrap" style={{ animation: 'marquee 120s linear infinite' }}>
        {loopWords.map((word, i) => (
          <span
            key={`${word}-${i}`}
            className="font-display text-2xl md:text-3xl tracking-widest text-ink-900 mx-8 flex items-center gap-6 flex-shrink-0"
          >
            {word}
